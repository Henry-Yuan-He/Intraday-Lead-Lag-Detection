...
def calculate_column_means(matrix):
    ...
    means = np.empty(n_rows, dtype=matrix.dtype)
    for i in range(n_rows):
        means[i] = np.sum(matrix[i, :]) / n_cols
    return means

@njit
def compute_total_lead_strength(matrix, set1, set2):
    total_strength = 0.0
    for i in set1:
        for j in set2:
            total_strength += matrix[i, j]
    return total_strength

@njit
def pca_numba(X, n_components):
    col_means = np.zeros(X.shape[1])
    for i in range(X.shape[1]):
        col_means[i] = np.sum(X[:, i]) / X.shape[0]
    
    X_meaned = X - col_means

    cov_matrix = np.dot(X_meaned.T, X_meaned) / X.shape[0]
...
@njit
def identify_leaders_followers_t2(lead_lag_matrix, rank_method='col_sum', top_percent=20, bottom_percent=20):
    if rank_method == 'col_sum':
        column_means = calculate_column_means(lead_lag_matrix)
        num_stocks = len(column_means)
        top_n = int(num_stocks * top_percent / 100)
        bottom_n = int(num_stocks * bottom_percent / 100)
        
        sorted_indices = np.argsort(column_means)
        leaders = sorted_indices[-top_n:] 
        followers = sorted_indices[:bottom_n]
    elif rank_method == 'greedy_search':
        column_means = calculate_column_means(lead_lag_matrix)
        num_stocks = len(column_means)
        candidate_size1 = int(num_stocks * 30 / 100)
        candidate_size2 = int(num_stocks * 30 / 100) 
        set_size1 = int(num_stocks * top_percent / 100)
        set_size2 = int(num_stocks * bottom_percent / 100)

        sorted_indices = np.argsort(column_means)
        candidate_leaders_indices = sorted_indices[-candidate_size1:].astype(np.int64)
        candidate_followers_indices = sorted_indices[:candidate_size2].astype(np.int64)

        best_set1 = np.zeros(set_size1, dtype=np.int64)
        best_set2 = np.zeros(set_size2, dtype=np.int64)
        best_total_lead_strength = -np.inf

        for i in candidate_leaders_indices:
            for j in candidate_followers_indices:
                set1 = np.array([i], dtype=np.int64)
                set2 = np.array([j], dtype=np.int64)
                remaining_leader_stocks = setdiff1d_numba(candidate_leaders_indices, set1)
                remaining_follower_stocks = setdiff1d_numba(candidate_followers_indices, set2)

                while len(set1) < set_size1 and len(set2) < set_size2:
                    max_lead_set1, max_lead_set2 = -1, -1
                    max_lead_value1, max_lead_value2 = -np.inf, -np.inf

                    for k in remaining_leader_stocks:
                        potential_set1_lead = np.sum(lead_lag_matrix[k, set2])
                        if potential_set1_lead > max_lead_value1:
                            max_lead_value1 = potential_set1_lead
                            max_lead_set1 = k

                    for k in remaining_follower_stocks:
                        potential_set2_lead = np.sum(lead_lag_matrix[set1, k])
                        if potential_set2_lead > max_lead_value2:
                            max_lead_value2 = potential_set2_lead
                            max_lead_set2 = k

                    if max_lead_set1 != -1 and len(set1) < set_size1:
                        set1 = np.append(set1, max_lead_set1)
                        remaining_leader_stocks = setdiff1d_numba(remaining_leader_stocks, np.array([max_lead_set1], dtype=np.int64))

                    if max_lead_set2 != -1 and len(set2) < set_size2:
                        set2 = np.append(set2, max_lead_set2)
                        remaining_follower_stocks = setdiff1d_numba(remaining_follower_stocks, np.array([max_lead_set2], dtype=np.int64))

                total_lead_strength = compute_total_lead_strength(lead_lag_matrix, set1, set2)

                if total_lead_strength > best_total_lead_strength:
                    best_total_lead_strength = total_lead_strength
                    best_set1 = set1
                    best_set2 = set2
...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method, update_interval, rank_method):
    ...
    plt.grid(True)
    plt.savefig(f'./results_greedy_1min/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}_{rank_method}.pdf')
    plt.close()
...
def process_combination(args):
    ...
    update_interval_t, update_interval_label = update_interval
    file_path = f'./results_greedy_1min/returns_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}_{rank_method}.pkl'
...
if __name__ == '__main__':
    ...
    rank_methods = ['greedy_search']

    combinations = [(df, bucket, lookback_window, method, update_interval, rank_method)
