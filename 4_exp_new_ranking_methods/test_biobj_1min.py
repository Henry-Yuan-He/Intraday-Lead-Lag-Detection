...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method, update_interval, rank_method):
    ...
    plt.grid(True)
    plt.savefig(f'./results_biobj_1min/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}_{rank_method}.pdf')
    plt.close()
...
def process_combination(args):
    ...
    update_interval_t, update_interval_label = update_interval
    file_path = f'./results_biobj_1min/returns_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}_{rank_method}.pkl'
