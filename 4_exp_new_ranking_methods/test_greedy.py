...
def identify_leaders_followers_t2(lead_lag_matrix, rank_method='col_sum', top_percent=20, bottom_percent=20):
    if rank_method == 'col_sum':
        ...
        column_means = calculate_column_means(lead_lag_matrix)
        num_stocks = len(column_means)
        candidate_size1 = int(num_stocks * 50 / 100)
        candidate_size2 = int(num_stocks * 50 / 100) 
...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method, rank_method):
    ...
    plt.grid(True)
    plt.savefig(f'./results_greedy/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}_{rank_method}.pdf')
    plt.close()

    
def process_combination(args):
    df, bucket, lookback_window, method, rank_method = args
    file_path = f'./results_greedy/returns_bucket{bucket}_lookback_window{lookback_window}_{method}_{rank_method}.pkl'
