...
def identify_leaders_followers_t2(lead_lag_matrix, rank_method='kmeans', top_percent=20, bottom_percent=20):
    if rank_method == 'kmeans':
        # Step 1: Sort each row from large to small
        sorted_matrix = np.sort(lead_lag_matrix, axis=1)[:, ::-1]

        # Step 2: Calculate cosine similarity between vectors
        similarity_matrix = cosine_similarity(sorted_matrix)
...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method, update_interval, rank_method):
    ...
    plt.grid(True)
    plt.savefig(f'./results_sort_kmeans_1min/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}_{rank_method}.pdf')
    plt.close()
...
def process_combination(args):
    ...
    update_interval_t, update_interval_label = update_interval
    file_path = f'./results_sort_kmeans_1min/returns_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}_{rank_method}.pkl'
