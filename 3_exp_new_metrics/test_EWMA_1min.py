...
def get_lead_lag_matrix_t(df, lookback_window, method, t, alpha, prev_lead_lag_matrix=None):
    tickers = df.columns
    ...
    if prev_lead_lag_matrix is not None:
        lead_lag_matrix = alpha * lead_lag_matrix + (1 - alpha) * prev_lead_lag_matrix
...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method, update_interval, alpha):
    ...
    plt.grid(True)
    plt.savefig(f'./results_EWMA_1mins/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}_alpha{alpha}.pdf')
    plt.close()

    
def process_combination(args):
    df, bucket, lookback_window, method, update_interval, alpha = args
    update_interval_t, update_interval_label = update_interval
    file_path = f'./results_EWMA_1mins/returns_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}_alpha{alpha}.pkl'
    try:
        print(f"Start bucket: {bucket}, lookback_window: {lookback_window}, method: {method}, update_interval_t{int(update_interval_t)}_label{update_interval_label}_alpha{alpha}.")
        data = aggregate_log_return(df.copy(), bucket)
        data['datetime'] = pd.to_datetime(data['date'] + ' ' + data['time'])
        data.set_index('datetime', inplace=True)
        data.drop(columns=['date', 'time'], inplace=True)
        if not os.path.exists(file_path):  # skip already processed parameters
            ...
            for t in tqdm(range(lookback_window, len(data)-1)):
                ...
                if t == lookback_window or (t - last_update_t) >= update_interval_t:
                    lead_lag_matrix = get_lead_lag_matrix_t(data, lookback_window, method, t, alpha, prev_lead_lag_matrix)
                    last_update_t = t  # Update the last update time step
                    prev_lead_lag_matrix = lead_lag_matrix
            ...
            if not strategy_returns:
                print(f"Skip bucket: {bucket}, lookback_window: {lookback_window}, method: {method}, update_interval_t{int(update_interval_t)}_label{update_interval_label}_alpha{alpha}.")
                return
        
            pd.Series(strategy_returns).to_pickle(file_path)
        else:
            strategy_returns = pd.read_pickle(file_path)
        plot_cumulative_returns(strategy_returns, data_index=data.index, bucket=bucket, lookback_window=lookback_window, method=method, update_interval=update_interval, alpha=alpha)
        print(f"Finish bucket: {bucket}, lookback_window: {lookback_window}, method: {method}, update_interval_t{int(update_interval_t)}_label{update_interval_label}_alpha{alpha}.")
...
if __name__ == '__main__':
    ...
    combinations = [(df.copy(), bucket, lookback_window, method, update_interval, alpha)
    ...
    print(f'num of cpus: {os.cpu_count()}')
