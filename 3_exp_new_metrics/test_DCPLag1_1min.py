...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method, update_interval):
    ...
    plt.grid(True)
    plt.savefig(f'./results_DCPLag1_1min/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}.pdf', 
                bbox_inches='tight')
    plt.close()
...
def process_combination(args):
    ...
    update_interval_t, update_interval_label = update_interval
    file_path = f'./results_DCPLag1_1min/returns_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}.pkl'
