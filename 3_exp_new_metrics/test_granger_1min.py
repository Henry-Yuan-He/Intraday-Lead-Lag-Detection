...
def get_lead_lag_matrix_t_granger(df, lookback_window, method, t):
    ...
    valid_columns = np.std(current_data, axis=0) > 1e-6

    for i in range(num_tickers):
        for j in range(num_tickers):
            if i != j and valid_columns[i] and valid_columns[j]:
                try:
                    ...
                    print(f"Error processing ticker {tickers[i]} and {tickers[j]}: {e}")
                    granger_matrix[i, j] = 0
...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method, update_interval):
    ...
    plt.grid(True)
    plt.savefig(f'./results_granger_1min/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}.pdf', 
                bbox_inches='tight')
    plt.close()
...
def process_combination(args):
    ...
    update_interval_t, update_interval_label = update_interval
    file_path = f'./results_granger_1min/returns_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}.pkl'
