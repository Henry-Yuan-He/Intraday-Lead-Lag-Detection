...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method):
    ...
    plt.grid(True)
    plt.savefig(f'./results_DCPLag1/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}.pdf')
    plt.close()

    
def process_combination(args):
    df, bucket, lookback_window, method = args
    file_path = f'./results_DCPLag1/returns_bucket{bucket}_lookback_window{lookback_window}_{method}.pkl'
