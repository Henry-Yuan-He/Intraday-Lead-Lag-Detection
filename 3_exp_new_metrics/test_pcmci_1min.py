...
def compute_lead_lag_with_numba(data_subset, method):
    ...
    if method == 'ccf_at_lag': # paper 1, eq 1
        ...
        same_direction = (Rp_lag * Rq_shifted) > 0

        probability = np.sum(same_direction) / len(same_direction)
        lead_lag_measure = probability
        return lead_lag_measure
    else:
...
def get_lead_lag_matrix_t_pcmci(df, lookback_window, method, t):
    tickers = df.columns
    ...
    dataframe = DataFrame(current_data.values, var_names=current_data.columns)
    ...
    pcmci_matrix = val_matrix[:,:,1] * (p_matrix[:, :, 1]<0.05)
    lead_lag_matrix = pcmci_matrix - pcmci_matrix.T
...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method, update_interval):
    ...
    plt.title(f'annualized return: {annualized_return*100:.3f}%, annualized volatility: {annualized_volatility*100:.3f}%, sharpe ratio: {sharpe_ratio:.3f}', fontsize=12)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.grid(True)
    plt.savefig(f'./results_pcmci_1min/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}.pdf', 
                bbox_inches='tight')
    plt.close()

    
def process_combination(args):
    df, bucket, lookback_window, method, update_interval = args
    update_interval_t, update_interval_label = update_interval
    file_path = f'./results_pcmci_1min/returns_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}.pkl'
    ...
    try:
        ...
        for t in tqdm(range(lookback_window, len(data)-1)):
            ...
            if t == lookback_window or (t - last_update_t) >= update_interval_t:
                lead_lag_matrix = get_lead_lag_matrix_t_pcmci(data, lookback_window, method, t)
        ...
        plot_cumulative_returns(strategy_returns, data_index=data.index, bucket=bucket, lookback_window=lookback_window, method=method, update_interval=update_interval)
...
if __name__ == '__main__':
    ...
    combinations = [(df.copy(), bucket, lookback_window, method, update_interval)
