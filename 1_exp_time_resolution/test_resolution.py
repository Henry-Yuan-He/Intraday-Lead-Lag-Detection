...
def backtest_strategy(strategy_returns, bucket):
    ...
    trading_times_per_year = unit_to_year[bucket]
    strategy_returns = np.array(strategy_returns)
    
    cumulative_return = np.prod(1 + strategy_returns) - 1

    n_periods = len(strategy_returns) / trading_times_per_year
    annualized_return = (1 + cumulative_return) ** (1 / n_periods) - 1
    
    annualized_volatility = np.std(strategy_returns) * np.sqrt(trading_times_per_year)
...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method):
    ...
    plt.grid(True)
    plt.savefig(f'./results_resolutions/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}.pdf')
    plt.close()

    
def process_combination(args):
    df, bucket, lookback_window, method = args
    file_path = f'./results_resolutions/returns_bucket{bucket}_lookback_window{lookback_window}_{method}.pkl'
    try:
...
if __name__ == '__main__':
    ...
    buckets = ['1min', '5min', '10min', '30min', '1h', '3h', '1d', '2d', '7d', '14d', '21d', '30d']
    lookback_window_dict = {
        '1min': [10, 30, 60, 100, 200, 500, 1000, 2500, 5000, 10000], #[0. ,  0.1,  0.2,  0.3,  0.5,  1.3,  2.6,  6.4, 12.8, 25.6]d
        '5min': [10, 20, 30, 60, 100],#[10, 20, 30, 60, 100, 200, 300, 500, 1000, 2500], #[0.1,  0.3,  0.4,  0.8,  1.3,  2.6,  3.8,  6.4, 12.8, 32.1]d
        '10min': [10, 20, 30, 60, 100, 200, 300, 500, 1000, 1500], #[0.3,  0.5,  0.8,  1.5,  2.6,  5.1,  7.7, 12.8, 25.6, 38.5]d
        '30min': [10, 20, 30, 60, 100, 200, 300, 500, 1000, 1500], #[0.8,  1.5,  2.3,  4.6,  7.7, 15.4, 23.1, 38.5, 76.9, 115.4]
        '1h': [10, 20, 30, 60, 100, 200, 300, 500, 1000, 1500], #[1.5,   3.1,   4.6,   9.2,  15.4,  30.8,  46.2,  76.9, 153.8, 230.8]
        '3h': [10, 20, 30, 60, 100, 200, 300, 500, 1000, 1500], #[4.6,   9.2,  13.8,  27.7,  46.2,  92.3, 138.5, 230.8, 461.5, 692.3]
        '1d': [5, 10, 30, 50, 100, 200, 300, 500, 700, 1000],
        '2d': [5, 10, 20, 30, 50, 80, 100, 200, 300, 500], #[10,   20,   40,   60,  100,  160,  200,  400,  600, 1000]d
        '7d': [5, 10, 20, 30, 40, 50, 80, 100, 120, 150], #[35,   70,  140,  210,  280,  350,  560,  700,  840, 1050]d
        '14d': [3, 5, 10, 20, 30, 40, 50, 60, 80, 100], #[42,   70,  140,  280,  420,  560,  700,  840, 1120, 1400]d
        '21d': [3, 5, 10, 15, 20, 30, 40, 50, 60, 70], #[63,  105,  210,  315,  420,  630,  840, 1050, 1260, 1470]d
        '30d': [3, 5, 10, 15, 20, 25, 30, 35, 40, 50] #[90,  150,  300,  450,  600,  750,  900, 1050, 1200, 1500]d
    }
    methods = ['levy_area', 'ccf_at_lag']
