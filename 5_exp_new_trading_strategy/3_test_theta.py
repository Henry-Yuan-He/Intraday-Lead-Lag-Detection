...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method, quantile):
    ...
    plt.grid(True)
    plt.savefig(f'./results_theta/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}_quantile{quantile}.pdf')
    plt.close()

    
def process_combination(args):
    df, bucket, lookback_window, method, quantile = args
    file_path = f'./results_theta/returns_bucket{bucket}_lookback_window{lookback_window}_{method}_quantile{quantile}.pkl'
    if os.path.exists(file_path): 
        return

    with open('./leaders_average_returns_over_t.pkl', 'rb') as file:
    ...
    try:
        ...
        plot_cumulative_returns(strategy_returns, data_index=data.index, bucket=bucket, lookback_window=lookback_window, method=method, quantile=quantile)
...
if __name__ == '__main__':
    ...
    methods = ['levy_area']
    quantiles = np.arange(0, 0.81, 0.01)

    combinations = [(df, bucket, lookback_window, method, quantile) 
