...
def process_combination(args):
    ...
    try:
        ...
        market_returns = data['SPY']  # Assuming SPY is the market index ticker

        strategy_returns = []
        leaders_followers_records = {'t': [], 'leaders': [], 'followers': []}
        ...
        for t in tqdm(range(lookback_window, len(data)-1)):
            ...
            # Record leaders and followers
            leaders_followers_records['t'].append(t)
            leaders_followers_records['leaders'].append(leaders)
            leaders_followers_records['followers'].append(followers)
        
        # pd.Series(strategy_returns).to_pickle(file_path)
        pd.to_pickle(leaders_followers_records, leaders_followers_file)

        # plot_leaders_followers_returns(data, leaders_followers_records, bucket, lookback_window, method)
        # plot_leader_follower_modes(data, leaders_followers_records, bucket, lookback_window, method)
        # plot_leader_market_modes(data, leaders_followers_records, bucket, lookback_window, method)
...
def plot_leaders_followers_returns(df, leaders_followers_records, bucket, lookback_window, method):
    leaders_avg_returns = []
    followers_avg_returns = []
    years = []
    
    for t, leaders, followers in zip(leaders_followers_records['t'], leaders_followers_records['leaders'], leaders_followers_records['followers']):
        leaders_avg_returns.append(df.iloc[t][leaders].mean())
        followers_avg_returns.append(df.iloc[t + 1][followers].mean())
        years.append(df.index[t].year + (df.index[t].day_of_year / 365.25))

    leaders_avg_returns = np.array(leaders_avg_returns)
    followers_avg_returns = np.array(followers_avg_returns)

    leaders_ci = 1.96 * np.std(leaders_avg_returns) / np.sqrt(len(leaders_avg_returns))
    followers_ci = 1.96 * np.std(followers_avg_returns) / np.sqrt(len(followers_avg_returns))
    
    plt.figure(figsize=(10, 6))
    plt.plot(years, leaders_avg_returns, label='Leaders Avg Return')
    plt.fill_between(years, leaders_avg_returns - leaders_ci, leaders_avg_returns + leaders_ci, alpha=0.3)
    plt.plot(years, followers_avg_returns, label='Followers Avg Return')
    plt.fill_between(years, followers_avg_returns - followers_ci, followers_avg_returns + followers_ci, alpha=0.3)
    plt.xlabel('Years')
    plt.ylabel('Average Return')
    plt.title(f'Leaders vs Followers Avg Returns\nBucket: {bucket}, Lookback Window: {lookback_window}, Method: {method}')
    plt.legend()
    plt.grid(True)
    plt.savefig(f'./leaders_followers_returns_bucket{bucket}_lookback_window{lookback_window}_{method}.pdf')
    plt.close()

def plot_leader_follower_modes(df, leaders_followers_records, bucket, lookback_window, method):    
    modes = []
    years = []

    for t, leaders, followers in zip(leaders_followers_records['t'], leaders_followers_records['leaders'], leaders_followers_records['followers']):
        leaders_avg_return = df.iloc[t][leaders].mean()
        followers_avg_return = df.iloc[t + 1][followers].mean()
        years.append(df.index[t].year + (df.index[t].day_of_year / 365.25))
        
        if leaders_avg_return > 0 and followers_avg_return > 0:
            modes.append(2)
        elif leaders_avg_return < 0 and followers_avg_return < 0:
            modes.append(1)
        elif leaders_avg_return > 0 and followers_avg_return < 0:
            modes.append(-2)
        elif leaders_avg_return < 0 and followers_avg_return > 0:
    ...
    plt.scatter(years, modes, marker='o', s=10, zorder=3)
    plt.yticks([2, 1, -1, -2], ['$R_{t-1}^{leader} > 0$,\n$R_t^{follower} > 0$', 
                                 '$R_{t-1}^{leader} < 0$,\n$R_t^{follower} < 0$', 
                                 '$R_{t-1}^{leader} < 0$,\n$R_t^{follower} > 0$', 
                                 '$R_{t-1}^{leader} > 0$,\n$R_t^{follower} < 0$'])
    plt.xlabel('Years')
    plt.ylabel('Mode of $R_{t-1}^{leader}$ and $R_t^{follower}$')
    plt.title(f'Modes of $R_{{t-1}}^{{leader}}$ and $R_{{t}}^{{follower}}$\nBucket: {bucket}, Lookback Window: {lookback_window}, Method: {method}')
    

    # Save the figure as a PDF
    pdf_filename = f'./leader_follower_modes_bucket{bucket}_lookback_window{lookback_window}_{method}.pdf'
    plt.savefig(pdf_filename)
    plt.close()

    print(f"Figure saved as {pdf_filename}")

def plot_leader_market_modes(df, leaders_followers_records, bucket, lookback_window, method):
    modes = []
    years = []

    for t, leaders in zip(leaders_followers_records['t'], leaders_followers_records['leaders']):
        leaders_avg_return = df.iloc[t][leaders].mean()
        market_return = df.iloc[t + 1]['SPY']
        years.append(df.index[t].year + (df.index[t].day_of_year / 365.25))
        
        if leaders_avg_return > 0 and -market_return > 0:
            modes.append(2)
        elif leaders_avg_return < 0 and -market_return < 0:
            modes.append(1)
        elif leaders_avg_return > 0 and -market_return < 0:
            modes.append(-2)
        elif leaders_avg_return < 0 and -market_return > 0:
    ...
    plt.yticks([2, 1, -1, -2], ['$R_{t-1}^{leader} > 0,$\n$-R_t^{market} > 0$', 
                                 ...
                                 '$R_{t-1}^{leader} > 0,$\n$-R_t^{market} < 0$'])
    plt.xlabel('Years')
    plt.ylabel('Mode of $R_{t-1}^{leader}$ and $R_t^{market}$')
    plt.title('Modes of $R_{{t-1}}^{{leader}}$ and $R_{{t}}^{{market}}$\nBucket: {bucket}, Lookback Window: {lookback_window}, Method: {method}')
    

    # Save the figure as a PDF
    pdf_filename = f'./leader_market_modes_bucket{bucket}_lookback_window{lookback_window}_{method}.pdf'
    plt.savefig(pdf_filename)
    plt.close()
