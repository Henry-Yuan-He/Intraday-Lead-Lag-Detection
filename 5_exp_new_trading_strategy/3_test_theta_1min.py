...
def execute_trades_t(df, leaders, followers, market_returns, t, theta_cut_pos, theta_cut_neg):
    ...
    if leaders_return >= theta_cut_pos:
        portfolio_return =  followers_return - market_return
    elif leaders_return <= theta_cut_neg:
...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method, update_interval, quantile):
    ...
    plt.grid(True)
    plt.savefig(f'./results_theta_1min/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}_quantile{quantile}.pdf')
    plt.close()

    
def process_combination(args):
    df, bucket, lookback_window, method, update_interval, quantile = args
    update_interval_t, update_interval_label = update_interval
    file_path = f'./results_theta_1min/returns_bucket{bucket}_lookback_window{lookback_window}_{method}_update_interval_t{int(update_interval_t)}_label{update_interval_label}_quantile{quantile}.pkl'
    if os.path.exists(file_path):  # skip already processed parameters
        return

    with open('./leaders_average_returns_over_t_1min.pkl', 'rb') as file:
        leaders_average_returns_over_t = pickle.load(file)

    positive_returns = [x for x in leaders_average_returns_over_t if x > 0]
    negative_returns = [x for x in leaders_average_returns_over_t if x < 0]

    theta_cut_pos = np.quantile(positive_returns, quantile)
    theta_cut_neg = -np.quantile(np.abs(negative_returns), quantile)
    ...
    try:
        ...
        for t in tqdm(range(lookback_window, len(data)-1)):
            ...
            strategy_returns.append(execute_trades_t(data, leaders, followers, market_returns,t, theta_cut_pos, theta_cut_neg))
        ...
        plot_cumulative_returns(strategy_returns, data_index=data.index, bucket=bucket, lookback_window=lookback_window, method=method, update_interval=update_interval, quantile=quantile)
...
if __name__ == '__main__':
    ...
    quantiles = np.arange(0, 0.91, 0.01)

    combinations = [(df, bucket, lookback_window, method, update_interval, quantile)
