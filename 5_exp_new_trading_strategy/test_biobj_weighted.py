...
def normalize_weights(weights, min_alpha=0.2, max_alpha=0.9):
    ...
    if len(non_zero_weights) > 0:
        ...
        weights = normalized_weights * (max_alpha - min_alpha) + min_alpha
        weights[weights == 0] = 0
    return weights


def plot_leaders_followers(row_sums, positive_counts, leaders, followers, leader_weights, follower_weights, t):
    plt.figure(figsize=(6, 4))
    
    # Normalize and adjust the weights
    leader_weights = normalize_weights(leader_weights, 0.2, 0.9) ** 1.5
    follower_weights = normalize_weights(follower_weights, 0.2, 0.9) ** 1.5

    # Add grid first with lower zorder
    plt.grid(True, zorder=1)

    # All others (non-leaders, non-followers) in gray
    others = set(range(len(row_sums))) - set(leaders) - set(followers)
    plt.scatter(row_sums, positive_counts, color='gray', alpha=0.5, zorder=2)

    # Leaders in shades of red
    for i, leader in enumerate(leaders):
        color = (1.0, 1.0 - leader_weights[i], 1.0 - leader_weights[i])  # Red gradient
        plt.scatter(row_sums[leader], positive_counts[leader], color=color, edgecolor='black', zorder=3)
    
    # Followers in shades of blue
    for i, follower in enumerate(followers):
        color = (1.0 - follower_weights[i], 1.0 - follower_weights[i], 1.0)  # Blue gradient
        plt.scatter(row_sums[follower], positive_counts[follower], color=color, edgecolor='black', zorder=3)
    
    # Add representative points for legend
    plt.scatter([], [], color='red', edgecolor='black', label='Leaders', zorder=3)
    plt.scatter([], [], color='blue', edgecolor='black', label='Followers', zorder=3)
    plt.scatter([], [], color='gray', alpha=0.5, label='Others', zorder=3)

    # Set labels and title
    plt.xlabel('RowSum Score', fontsize=14)
    plt.ylabel('Leading Count', fontsize=14)
    ...
    plt.legend()
    # plt.grid(True)
    plt.savefig(f'./ndfronts_weighted{t}.pdf', bbox_inches='tight')
    plt.close()
...
# @njit
def identify_leaders_followers_t2(lead_lag_matrix, rank_method='biobj2', top_percent=20, bottom_percent=20, t=None):
    if rank_method == 'biobj2':
        ...
        # if t % 55 == 0:
        #     plot_leaders_followers(row_sums, positive_counts, leaders, followers, leader_weights, follower_weights, t)
...
def plot_cumulative_returns(returns, data_index, bucket, lookback_window, method, rank_method):
    ...
    plt.grid(True)
    plt.savefig(f'./results_biobj_weighted/cum_return_bucket{bucket}_lookback_window{lookback_window}_{method}_{rank_method}.pdf')
    plt.close()

    
def process_combination(args):
    df, bucket, lookback_window, method, rank_method = args
    file_path = f'./results_biobj_weighted/returns_bucket{bucket}_lookback_window{lookback_window}_{method}_{rank_method}.pkl'
    ...
    try:
        ...
        for t in tqdm(range(lookback_window, len(data)-1)):
            ...
            leaders_ind, followers_ind, leader_weights, follower_weights = identify_leaders_followers_t2(lead_lag_matrix, rank_method=rank_method, t=t)
